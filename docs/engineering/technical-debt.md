...
| TD-04 | CI runs on **ubuntu-latest only** (not macOS/matrix) | No iOS/native tooling yet; ubuntu is fast/cheap (Doc 15) | macOS/Windows-specific regressions unnoticed | When device tooling (simctl/adb) lands, or Win/Linux become committed targets | 🟨 **partially paid down 2026-07-26** (trigger fired — E-09 simctl landed): the **`e2e` job now runs on a `[ubuntu-latest, macos-latest]` matrix**, so a real Electron launch **and** the reaper's real `ps -o lstart=` / `kill(-pgid)` are validated on macOS — our first-class target (NG-7) — not just Linux. `verify` stays ubuntu-only (platform-agnostic: TS/lint/format/unit-with-injected-executors/bundle), and xvfb + `playwright install-deps` are gated to Linux (macOS has a display). Cost-aware: macOS runners bill ~10×, so only the platform-sensitive suite runs there. **Still open:** Windows (deferred until a committed target, TD-12) and running real `simctl` against a booted sim in CI (executors are injected/mocked today). |
...
| ID | Deferred item | Why deferred | Pay-down trigger | Status |
|----|---------------|--------------|------------------|--------|
| TD-11 | **Cross-launch reaper** for hard-crash orphans | Clean-exit teardown covers `disposeAll`; a `SIGKILL` of Icarus itself can't run cleanup, and detached children then survive | First real long-lived spawn (M1 Metro), or first reported orphan | 🟩 **resolved 2026-07-26** (trigger fired — Metro is the first long-lived spawn): `process/orphan-registry.ts` persists every POSIX process group we spawn to `userData/orphaned-processes.json` and reaps survivors at next launch. The **PID-recycle hazard** is handled by a stable per-process identity marker (OS start time via `ps -o lstart=`, `psIdentityProbe`): a survivor is force-killed **only** when its marker still matches — an unmarked/mismatched/dead record is never killed, so we can't murder an unrelated process that inherited the pid. `ProcessManager` takes an optional `OrphanRegistry` (records on spawn, clears on exit; win32 skipped, NG-7); desktop reaps in `whenReady` before any spawn. All I/O (persistence, probe, kill) is injected → pure reap logic. +15 tests incl. a real-process end-to-end reap (kills a live orphan, spares a marker-mismatched one). |
...
| TD-19 | **Persist the unified log to disk** (OQ-9) | Cross-cutting concern (logs, store, future replay); deferred to a unified persistence epic | Persistence epic (M2+) | 🟩 **resolved 2026-07-26** (persistence) + **2026-07-29** (opt-in export): `UnifiedLogPersistence` (core, Electron-free) keeps a **bounded recent tail** under `userData`, written on a **debounced** cadence (bounded I/O), restored into the live log on launch (`UnifiedLogController.replay`) and **cleared on a clean exit** — a crash is recoverable, a normal close leaves no durable debug-log footprint. Local-only; never transmitted. Resolves OQ-9 via [ADR-0012](../adr/ADR-0012-unified-log-persistence.md) (bounded/local/clear-on-exit, chosen over unbounded on-disk history for TR-5). New injected `FileStore` seam (mirrors the reaper's store, TD-11). +18 tests. **Export (M3 first slice, E-15)**: `buildLogExport` (core) + `LogExporter` (desktop) + `command:log.export` IPC + an "Export N entries" button in the unified-log panel. The renderer hands main its currently-visible entries (filter chips + search are the user's intent); main writes the file with the same `redact()` rules the E-12 AI boundary uses, so a planted secret never reaches the file (M3 canary in `log-export.test.ts` is the regression gate). Local-only, never transmitted; cancellation is a clean no-op. JSONL for `jq`/`grep` interop; default filename is timestamped UTC, never the project name. **Still explicitly deferred (and out-of-scope for v1):** network export, replay (loading a file back into the live log), alternative formats, any auto-export / cloud upload. Each is a follow-on with a clear trigger. |
