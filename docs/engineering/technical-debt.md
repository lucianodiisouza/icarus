...
|----|---------------|--------------|------------------|--------|
| TD-06 | **Automated security-flag assertions** (contextIsolation/sandbox/CSP/allowlist) not yet tested in CI | Asserting live `webPreferences` needs a launched Electron instance (no display here) | Next desktop Epic that adds an E2E harness | 🟨 **partial 2026-07-25**: the TD-07 E2E harness now asserts the renderer has **no Node globals** (`require`/`process` undefined) and **does** expose the narrow `window.icarus` bridge — i.e. `contextIsolation: true` + `nodeIntegration: false` are verified live. Still unasserted: `sandbox`, the CSP header, and the navigation/window-open allowlist. Those ride the same harness — a follow-up slice can add `app.evaluate()` checks against the Main process. |
