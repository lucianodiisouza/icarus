...
| TD-21 | **Metro + native-syslog not fanned into the unified log** | Only `unified.pushCdp` is wired (from the CDP session); `UnifiedLogController.pushMetro`/`pushNative` exist but have no caller, so the (now-live, per TD-15) unified panel shows CDP console entries only, not Metro output or iOS syslog | Discovered wiring TD-15; next E-10 completeness pass — wire `metro.onLog → unified.pushMetro` and the syslog source → `pushNative` | 🟥 planned |
