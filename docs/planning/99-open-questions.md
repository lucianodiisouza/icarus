...
| OQ-22 | In-app bridge design (Heap/Profiler + React tree/navigation/state): scope, install UX, versioning | 🟢 (partial) | — | **Partially resolved 2026-07-30 (E-22 / OQ-22 live bridge):** the live-push upgrade of the read-only nav + perf-hotspots inspectors shipped on `main`. A generic `BridgePoller` (core, Electron-free) evaluates a user-installed `globalThis` expression on an interval, JSON-diffs the result, and fires a typed delta. A `BridgeController` (desktop) owns the two pollers; the renderer subscribes to one delta event channel with a typed `kind: 'nav' | 'perf_hotspots'`. **Still open:** heap + JS-metrics live push (same poller shape, one new probe expression — trivial follow-up); per-window subscription routing (current design broadcasts to the focused window — fine for a single-window desktop app); a formal ADR for the install-UX contract (the snippet is currently shown in the read-only sections and the live section explains it inline). |
